...
# Page configuration
st.set_page_config(
    page_title="Version Manager - ResumeMasterAI",
    page_icon="📝",
    layout="wide"
)
...
with st.sidebar:
    ...
    sort_by = st.selectbox(
        "Sort By",
        ["Most Recent", "Oldest First", "Name (A-Z)", "Name (Z-A)"]
    )
    
    show_archived = st.checkbox("Show Archived", value=False)

# Main Content
tab1, tab2, tab3, tab4 = st.tabs(["📚 All Versions", "🔄 Compare", "📊 Analytics", "⚙️ Settings"])
...
with tab1:
    ...
    if not versions:
        st.info("📭 No versions yet. Upload your first resume version using the sidebar!")
    else:
        # Stats Overview
        ...
        for version in versions:
            ...
            with col2:
                ...
                if st.button("📥 Download", key=f"download_{version_id}"):
                    file_data = st.session_state.version_manager.get_version_file(version_id)
                    st.download_button(
                        label="Download File",
                        data=file_data,
...
with tab2:
    ...
    if len(st.session_state.selected_versions) < 2:
        ...
        st.success(f"✅ {len(st.session_state.selected_versions)} versions selected for comparison")
        ...
        with col1:
            version1_id = st.selectbox(
                "Version 1 (Older)",
                st.session_state.selected_versions,
                format_func=lambda x: st.session_state.version_manager.get_version_name(x)
            )
...
with tab3:
    ...
    if not versions:
        ...
        version_dates = [v.get('date', datetime.now().strftime('%Y-%m-%d')) for v in versions]
...
with tab4:
    ...
    with col1:
        ...
        max_versions = st.number_input("Max versions to keep", min_value=5, max_value=100, value=20)
        auto_archive = st.checkbox("Auto-archive old versions", value=False)
        archive_after_days = st.number_input("Archive after (days)", min_value=7, max_value=365, value=30)
